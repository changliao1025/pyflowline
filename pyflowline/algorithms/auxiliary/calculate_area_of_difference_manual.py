        ...
        for m in range(nFlowline):
            ...
            if m == 0:
                for k in range(nVertex):
                    x1 = pFlowline.aVertex[k].dLongitude_degree
                    y1 = pFlowline.aVertex[k].dLatitude_degree
                    ring.AddPoint(x1, y1)
                    lons.append( x1 )
                    lats.append( y1 )
            else:
                iFlag_reverse =0 #if a flowline is reverse
                if iFlag_reverse ==0 :
                    for k in range(1,nVertex):
                        x1 = pFlowline.aVertex[k].dLongitude_degree
                        y1 = pFlowline.aVertex[k].dLatitude_degree
                        ring.AddPoint(x1, y1)
                        lons.append( x1 )
            ...
            #add the last one
            x1 = pFlowline.aVertex[0].dLongitude_degree
            y1 = pFlowline.aVertex[0].dLatitude_degree
            ring.AddPoint(x1, y1)
            lons.append( x1 )
            lats.append( y1 )
