...
def process_flowline(aFlowline_in, pVertex_outlet_initial, sWorkspace_output_basin,
    ...
    if iFlag_remove_duplicate_river == 1:
        try:
            ...
            ptimer.start()
            aFlowline_basin_conceptual = pRivergraph.remove_duplicate_flowlines(iFlag_direction_insensitive=iFlag_direction_insensitive)
            ptimer.stop()
            if iFlag_debug == 1:
                sTime = ptimer.now()
    ...
    try:
        ...
        if iFlag_debug ==1:
            sTime = ptimer.now()
            sFilename_out = os.path.join(sWorkspace_output_basin, 'flowline_segment_index_' + sTime + '.geojson')
    ...
    if iFlag_break_by_distance == 1:
        try:
            ...
            ptimer.start()
            aFlowline_basin_simplified_split = split_flowline_by_length(aFlowline_basin_simplified, dThreshold_break_by_distance)
            ptimer.stop()
            if iFlag_debug ==1:
                sTime = ptimer.now()
    ...
    return aFlowline_basin_simplified, aConfluence_basin_simplified, aStream_segment, aStream_order