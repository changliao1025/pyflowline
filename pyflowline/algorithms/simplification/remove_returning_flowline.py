...
def remove_returning_flowline(iMesh_type_in, aCell_intersect_in, pVertex_outlet_in):
    ...
    def retrieve_flowline_intersect_index(
        iSegment_in, iStream_order_in, pVertex_end_in
    ):
        lCellID = -1
        iFlag_found = 1
        ...
        while iFlag_found == 1:
            ...
            for j in range(nCell):
                ...
                for i in range(nFlowline):
                    ...
                    if (
                        pVertex_end == pVertex_end_current
                    ):  # be careful because this function is strict
                        if iStream_segment == iSegment_in:
                            dLength = pFlowline.dLength
                            iFlag_found2, dummy2 = pCell.which_edge_cross_this_vertex(
                                pVertex_start
                            )
                            iFlag_found3, dummy3 = pCell.which_edge_cross_this_vertex(
                                pVertex_end
                            )
        ...
        if iFlag_found == 0:
            ...
            for i in sort_index:
                retrieve_flowline_intersect_index(
                    aSegment_upstream[i], aStream_order[i], aVertex_end_upstream[i]
                )
                pass
            pass
