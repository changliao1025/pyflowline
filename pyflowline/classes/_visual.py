...
def plot(self, iFlag_type_in=None, iFlag_title_in=None, sVariable_in=None, **kwargs):
    """_summary_
        ...
        if iFlag_type_in == 2:
            ...
            if self.iMesh_type == 4:
                ...
                if self.dResolution_meter > 1000:
                    sResolution = (
                        "Resolution: "
                        + "{:0d}".format(int(self.dResolution_meter / 1000))
                        + " km"
                    )
                else:
                    sResolution = (
                        "Resolution: "
                        + "{:0d}".format(int(self.dResolution_meter))
                        + " m"
                    )
            ...
            if iFlag_type_in == 3:  # polygon based
                ...
                if iFlag_type_in == 4:  # mixed based mesh + flowline
                    if sVariable_in == "overlap":
                        self._plot_mesh_with_flowline(
                            iFlag_title_in=iFlag_title_in, **kwargs
                        )
...
def _plot_study_area(
    self, sFilename_boundary_in=None, sFilename_slope_in=None, sFilename_nhd_in=None
):
    return


# this is a reserved function
def _compare_with_raster_dem_method(
    self, sFilename_dem_flowline, sFilename_in, aExtent_in=None, pProjection_map_in=None
):
