...
def replace_last_occurrence(sFilename_path_in, sSubstring_in, sSubstring_out):
    ...
    if last_occurrence_index == -1:
        ...
        return (
            sFilename_path_in[:last_occurrence_index]
            + sSubstring_out
            + sFilename_path_in[last_occurrence_index + len(sSubstring_in) :]
        )
    ...
    **kwargs
    ...
    if iFlag_type_in == 1:
        ...
        if iFlag_type_in == 2:  # polyline based
            self._plot_polyline_variable(sVariable_in, **kwargs)
            pass
        else:
            if iFlag_type_in == 3:  # polygon based
                self._plot_polygon_variable(
                    sVariable_in, sFilename_boundary_in=sFilename_boundary_in, **kwargs
                )
                ...
                if iFlag_type_in == 4:  # mixed
                    if sVariable_in == "flow_direction_with_mesh":
                        ...
                        )  # this can be either domain wide or subbasin level
                        if sFilename_boundary_in is not None:
                            aFiletype_in = [3, 3, 2]
                            aFilename_in = [
                                sFilename_mesh,
                                sFilename_boundary_in,
                                sFilename,
                            ]
                            map_multiple_vector_files(
                                aFiletype_in,
                        ...
                        if sVariable_in == "flow_direction_with_observation":
                            ...
                            sFilename1 = replace_last_occurrence(
                                sFilename_dummy, "hexwatershed", "pyflowline"
                            )
                            sFilename_dummy = self.sFilename_flowline_conceptual
                            # now replace the folder string
                            sFilename2 = replace_last_occurrence(
                                sFilename_dummy, "hexwatershed", "pyflowline"
                            )
                            # check if it exist
                            if (
                                os.path.exists(sFilename1) == True
                                and os.path.exists(sFilename2) == True
                            ):
                                ...
                                if sFilename_boundary_in is not None:
                                    ...
                                    print(
                                        "Boundary file is needed for flow direction with observation."
                                    )
                                    pass
                        else:
                            ...
                            if sVariable_in == "travel_distance":
                                ...
                                if sVariable_in == "slope":
                                    ...
                                    if sVariable_in == "area_of_difference":
                                        ...
                                        iFiletype = 1
                                        sFilename_dummy = (
                                            self.sFilename_area_of_difference
                                        )
                                        # now replace the folder string
                                        sFilename = replace_last_occurrence(
                                            sFilename_dummy,
                                            "hexwatershed",
                                            "pyflowline",
                                        )
