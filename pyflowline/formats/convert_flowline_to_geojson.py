...
from pyflowline.formats.read_flowline import (
    read_flowline_shapefile,
    read_flowline_shapefile_swat,
    read_flowline_geojson,
)
from pyflowline.formats.export_flowline import export_flowline_to_geojson


def convert_flowline_to_geojson(sFilename_in, sFilename_geojson_out):
    ...
    if iFlag_type_in == 1:  # shapefile
        ...
        export_flowline_to_geojson(
            ...
            aAttribute_field=["lineid"],
            aAttribute_dtype=["int"],
            aAttribute_data=[aFlowlineID],
        )
    else:
        if iFlag_type_in == 2:
            sFilename_dummy = sFilename_geojson_out
            sFilename_out = sFilename_dummy.replace(".geojson", "_withID.geojson")
            aFlowline_basin, pProjection_geojson = read_flowline_geojson(
                sFilename_in, sFilename_out=sFilename_out
            )
            # get lineid
            aFlowlineID = [pFlowline.lFlowlineID for pFlowline in aFlowline_basin]
            # convert it
...
def convert_shapefile_to_geojson(
    iFlag_type_in, sFilename_shapefile_in, sFilename_geojson_in
):
    """Convert a shapefile to a json file
    ...
    if iFlag_type_in == 0:
        ...
        if iFlag_type_in == 1:
            aFlowline_basin, pSpatial_reference = read_flowline_shapefile(
            ...
            pSpatial_reference_gcs.SetAxisMappingStrategy(
...
def convert_shapefile_to_geojson_swat(
    iFlag_type_in, sFilename_shapefile_in, sFilename_geojson_in
):
    """Convert a shapefile to a json file
    ...
    if iFlag_type_in == 0:
        ...
        if iFlag_type_in == 1:
            aFlowline_basin, pSpatial_reference = read_flowline_shapefile_swat(
                sFilename_shapefile_in
            )
            iFlag_projected = 0
            pSpatial_reference_gcs = osr.SpatialReference()
