...
def export_flowline_to_shapefile(
    iFlag_projected_in,
    aFlowline_in,
    ...
    if (
        aAttribute_field is not None
        and aAttribute_data is not None
        and aAttribute_dtype is not None
    ):
        iFlag_attribute = 1
        ...
        if (
            nAttribute3 != nAttribute1
            or nAttribute1 != nAttribute2
            or nFlowline != nAttribute4
        ):
            print("The attribute is not correct, please check!")
            return
        else:
    ...
    pLayer_shapefile = pDataset_shapefile.CreateLayer(
        "flowline", pSpatial_reference_in, ogr.wkbLineString
    )
    # Add one attribute
    pLayer_shapefile.CreateField(
