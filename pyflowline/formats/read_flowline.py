...
def read_flowline_geojson(sFilename_geojson_in, sFilename_out=None):
    ...
    if sFilename_out is not None:
        ...
        new_layer = new_dataSource.CreateLayer(
            layer.GetName(), layer.GetSpatialRef(), ogr.wkbMultiLineString
        )
    ...
    # convert to wkt
    pProjection_geojson = (
        pSpatialRef_geojson.ExportToWkt()
    )  # by default, geojson only supports one osr
