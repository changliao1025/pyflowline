...
def create_hexagon_mesh(
    ...
    def add_cell_into_list1(
        ...
        pHexagon = convert_gcs_coordinates_to_cell(
    ...
    if iFlag_fill_hole == 1:
        ...
        for pCell in aHexagon:
            aNeighbor_land = (
                pCell.aNeighbor_land
            )  # including both holes and maps land cutoff by boundary
        ...
        for pCell in aHexagon:
            if (
                pCell.nNeighbor_land_virtual == 1
            ):  # only one virtual land means it is likely next to a hole
                ...
                if iFlag_rotation_in == 0:
                    ...
                    dY_shift = 0.5 * dLength_edge * np.sqrt(3.0)  # long displacement
                    dX_spacing = dLength_edge * 1.5  # cell to cell displacement in x
                    dY_spacing = dLength_edge * np.sqrt(
                        3.0
                    )  # cell to cell displacement in y
                    if iColumn % 2 == 0:
                        # define a polygon here
                        x1 = xleft + (iColumn - 1) * dX_spacing
                        y1 = ybottom + (iRow - 2) * dY_spacing + dY_shift
        ...
        for pCell in aHexagon:
            ...
            pCell.aNeighbor_land_virtual = (
                aNeighbor_land_virtual_update  # for book keeping only
            )
