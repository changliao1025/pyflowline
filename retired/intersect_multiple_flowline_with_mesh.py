...
def intersect_multiple_flowline_with_mesh(iMesh_type_in, sFilename_mesh, aFilename_flowline, sFilename_output):
    ...
    nFlowline = len(aFilename_flowline)

    
    pDataset_out = pDriver_shapefile.CreateDataSource(sFilename_output)

    pLayerOut = pDataset_out.CreateLayer('flowline', pSpatialRef_mesh, ogr.wkbMultiLineString)
    # Add one attribute
    ...
    for i in range(nFlowline):
        sFilename_flowline = aFilename_flowline[i]

        if  os.path.exists(sFilename_flowline) : 
            pass
        else:
        ...
        for j in range (nfeature_mesh):
            ...
            if(pGeometrytype_mesh == 'POLYGON'):
                ...
                iFlag_intersected = 0 
                for k in range (nfeature_flowline):
